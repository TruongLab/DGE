...
cell_values <- c("hiPSC", "hMac")  # Replace with your cell identities!
...
col_data <- data.frame(
  ...
  condition = c("hMac", "hMac", "hMac", "hiPSC", "hiPSC", "hiPSC"))
...
col_data$condition <- factor(col_data$condition, levels = c("hiPSC", "hMac"))
...
# Save results
write.csv(as.data.frame(res), "../DGE_outputs/deseq2_results.csv", row.names = TRUE)
...
write.csv(res_sig, "../DGE_outputs/dge_significant_results.csv", row.names = TRUE)
