...
kallisto_data1 <- read.csv("Kall_annotated_kallisto_output.csv")
kallisto_data2 <- read.csv("Tomoki_Kall_annotated_kallisto_output.csv")
...
cell_values <- c("M0", "hMac")  # Replace with your SRA values
...
col_data <- data.frame(
  ...
  condition = c("hMac", "hMac", "hMac", "M0", "M0")  # Labels for each sample
)
...
write.csv(res_sig, "dge_significant_results.csv", row.names = TRUE)
...
EnhancedVolcano(
  ...
  pCutoff = 0.01,                      # Adjusted p-value cutoff
  ...
  ylim = c(0, 35)                      # Expand the y-axis
)

# Save the plot with more space
ggsave("volcano_plot_large.png", plot = last_plot(), width = 12, height = 10, dpi = 300)
```
